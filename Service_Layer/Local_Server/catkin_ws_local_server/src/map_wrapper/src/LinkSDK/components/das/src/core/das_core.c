...
#include <../src/LinkSDK/components/das/inc/das.h>

static const char DAS_VERSION[] = "das:1.2.0";
static const char DAS_PROFILE[] = "profile:rtos|LK";
        ...
        switch (ss->step) {
            case DAS_STEP_INFO: {
                ...
                if (ss->stepping_service->info) {
                    ...
                    if (ret == DAS_ERROR_GENERIC && strlen((char*)&(ss->report.data.infData.state)) == 0) {
                        ret = DAS_SUCCESS;
                        is_stepping = true;
                    }
            ...
            case DAS_STEP_ATTEST: {
                ...
                if (ss->stepping_service->attest) {
                    ret = ss->stepping_service->attest(
                            ...
                            sizeof(ss->report.data.atsFilData.path), 
                            (char*)&(ss->report.data.atsFilData.attribute),
                            sizeof(ss->report.data.atsFilData.attribute),
                            sum_ctx, 
                            service_state
                            );
                    if (ret != DAS_SUCCESS) {
